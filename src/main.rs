...
impl Account {
    pub fn connect(
        &self,
    ) -> Result<Connection<TlsStream<TcpStream>>, imap::error::Error> {
...
impl<T: Read + Write + imap::extensions::idle::SetReadTimeout> Connection<T> {
    pub fn handle(
        mut self,
        account: usize,
        mut tx: mpsc::Sender<Option<(usize, usize)>>,
    ) {
        loop {
    ...
    fn check(
        ...
        loop {
            ...
            last_notified = std::cmp::max(
                last_notified,
                uids.iter().cloned().max().unwrap_or(0),
            );

            let mut subjects = BTreeMap::new();
            if !uids.is_empty() {
                let uids: Vec<_> =
                    uids.into_iter().map(|v: u32| format!("{}", v)).collect();
                for msg_header in self
                    .socket
                    .uid_fetch(&uids.join(","), "RFC822.HEADER")?
                    .iter()
                    .filter_map(|msg| msg.header())
                {
                    match mailparse::parse_headers(msg_header) {
                        Ok((headers, _)) => {
                        ...
                        Err(e) => eprintln!(
                            "failed to parse headers of message: {:?}",
                            e
                        ),
                    }
                }
            ...
            if !subjects.is_empty() {
                if let Some(notificationcmd) =
                    &self.account.notification_command
                {
...
fn main() {
    ...
    let config = {
        let mut f = match File::open(config) {
            Ok(f) => f,
            Err(e) => {
                eprintln!("Could not open configuration file buzz.toml: {}", e);
                return;
            }
        };
    ...
    let accounts: Vec<_> = match config.as_table() {
                    ...
                    Some(Account {
                        ...
                        server: (
                            match t["server"].as_str() {
                                Some(v) => v.to_owned(),
                                None => {
                                    return parse_failed("server", "string")
                                }
                            },
                        ...
                        notification_command: t
                            .get("notificationcmd")
                            .and_then(|raw_v| match raw_v.as_str() {
        ...
        None => {
            eprintln!(
                "Could not parse configuration file buzz.toml: not a table"
            );
            return;
        }
    };
    ...
    if let Err(e) = app.set_icon_from_file(
        "/usr/share/icons/Faenza/stock/24/stock_disconnect.png",
    ) {
    ...
    if accounts.is_empty() {
        eprintln!("No accounts in config worked; exiting...");
        return;
    }

    // We have now connected
    app.set_icon_from_file(
        "/usr/share/icons/Faenza/stock/24/stock_connect.png",
    )
    .ok();
